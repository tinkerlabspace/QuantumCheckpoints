...
public class StatusCommand implements SubCommand {
    ...
    public void execute(@NotNull Player player, @NotNull String[] args) {
        ...
        MessageUtil.info(player, "§6QuantumCheckpoints Status:");
        MessageUtil.info(player, " §7Enabled: " + (enabled ? "§aYes" : "§cNo"));
        MessageUtil.info(player, " §7Penalty: " + (penalty ? "§aYes" : "§cNo"));
        MessageUtil.info(player, " §7Limit: §e" + limit + " per player");
        MessageUtil.info(player, " §7Cost: §e" + cost);
        MessageUtil.info(player, " §7Total checkpoints: §e" + total);
    }
}