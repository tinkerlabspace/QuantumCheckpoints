...
public class CostCommand implements SubCommand {
    ...
    public void execute(@NotNull CommandSender sender, @NotNull String[] args) {
        if (args.length == 0) {
            MessageUtil.info(sender, "Current cost: §e" + plugin.getConfigManager().getCostDescription());
            MessageUtil.info(sender, "Usage: /checkpoints cost <item> <amount>");
            MessageUtil.info(sender, "Use §e/checkpoints cost none§7 to make free.");
            return;
        }
        ...
        if (material == null) {
            MessageUtil.error(sender, "Unknown item: " + args[0]);
            return;
        }
        ...
        try {
            ...
            MessageUtil.success(sender, "Cost set to " + amount + "x " +
                    material.name().toLowerCase().replace("_", " ") + ".");
        } catch (NumberFormatException e) {
            MessageUtil.error(sender, "Invalid amount: " + args[1]);
        }
    }
